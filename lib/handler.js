...
const { createStore } = require('./kv-store');
...
async function seedState(store) {
  const existing = await store.get('app-data');
  ...
  const confirmed = await store.get('app-data');
...
async function saveState(store, state, verify) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    const saved = await store.get('app-data');
...
async function findUserByEmail(store, email) {
  for (let attempt = 0; attempt < 5; attempt++) {
    const state = await store.get('app-data');
...
function routePath(event) {
  ...
  if (!cleaned.startsWith('/api/')) {
    cleaned = cleaned.startsWith('/') ? `/api${cleaned}` : `/api/${cleaned}`;
  }
